...
TcpSocketBase::EnterRecovery ()
{
  ...
  NS_LOG_UNCOND ("time: "<<Simulator::Now().GetSeconds() <<"; node: "<< m_node->GetId() << " " << m_dupAckCount << " dupack. Enter fast recovery mode." <<
 ...
 // if(this->IsInitialized())
  //{
	  if (!calculateThroughputInitialized)
	  {
		  ...
		  std::ofstream traceAll ("Throughput_for_node_" +
				  std::to_string(this->GetNode()->GetId()) + ".data", std::ofstream::out | std::ofstream::trunc);
	  }
	  std::ofstream traceAll ("Throughput_for_node_" +
		  std::to_string(this->GetNode()->GetId()) + ".data", std::ofstream::out | std::ofstream::app);
...
TcpSocketBase::ReTxTimeout ()
{
  ...
  if (m_state <= ESTABLISHED && m_txBuffer->HeadSequence () >= m_tcb->m_highTxMark && m_txBuffer->Size () == 0)
    {
      NS_LOG_UNCOND ("Already Sent full data" << m_txBuffer->HeadSequence () << " " << m_tcb->m_highTxMark);
      return;
    }
