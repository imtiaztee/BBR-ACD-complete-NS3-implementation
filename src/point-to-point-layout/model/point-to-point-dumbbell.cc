                                                        ...
                                                        PointToPointHelper bottleneckHelper)
{
	  ...
	  //Imtiaz Mahmud's (imtiaz.tee@gmail.com) implementation
		...
		/*
		 * we can set different delay and bandwidth for leftLeaf-router0 by the following code
		 * edited by: Imtiaz Mahmud (imtiaz.tee@gmail.com)
		 */

		uint32_t delayBase = 20;	//in ms
		uint32_t delayIncreament = 0;		//in ms

		uint32_t accessBwBase = 40;	//in Mbps
		uint32_t accessBwIncreament = 0;	//in Mbps

	  std::vector <PointToPointHelper> pointToPointLeftLeaf;
	  for (uint32_t i=0; i<nLeftLeaf; i++)
	  {
		  uint32_t delay = delayBase + (delayIncreament*i);
		  uint32_t accessBw = accessBwBase + (accessBwIncreament*i);

		  std::string access_delay_left = std::to_string(delay) + "ms";
		  std::string access_bandwidth_left = std::to_string (accessBw) + "Mbps";

		  PointToPointHelper pointToPoint;
		  pointToPoint.SetDeviceAttribute    ("DataRate", StringValue (access_bandwidth_left));
		  pointToPoint.SetChannelAttribute   ("Delay", StringValue (access_delay_left));
		  pointToPointLeftLeaf.push_back(pointToPoint);
	  }
  ...
  //Imtiaz Mahmud's (imtiaz.tee@gmail.com) implementation
  std::vector <PointToPointHelper> :: iterator it;
  uint32_t p = 0;
  for (it = pointToPointLeftLeaf.begin(); it != pointToPointLeftLeaf.end(); it++, p++)
    {
	  PointToPointHelper tempLeftHelper = *it;
      NetDeviceContainer c = tempLeftHelper.Install (m_routers.Get (0),
                                                 m_leftLeaf.Get (p));
