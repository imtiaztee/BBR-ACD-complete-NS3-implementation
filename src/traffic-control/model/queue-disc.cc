...
QueueDisc::DoInitialize (void)
{
  ...
  //print.SetFunction (&QueueDisc::PrintToFile, this);
  //print.Schedule(Seconds(1.0));
}
...
void
QueueDisc::PrintToFile (void)
{
	Ptr <NetDevice> netDevice = this->GetNetDevice();
	Ptr <Node> node = netDevice->GetNode();

    if(!startPrintToFile)
    {
  	  startPrintToFile = true;
	  std::ofstream traceAll ("BufferDisk_trace_for_node_" +
			  std::to_string(node->GetId()) + ".data", std::ofstream::out | std::ofstream::trunc);
    }
	   std::ofstream traceAll ("BufferDisk_trace_for_node_" +
		  std::to_string(node->GetId()) + ".data", std::ofstream::out | std::ofstream::app);
	   ...
	   if (traceAll.is_open())
	   {
	      ...
	      // Here is a example of this.
		   ...
		   traceAll << this->GetCurrentSize().GetValue() << "\t";	//as the name suggests, it is expected cwnd calculated from bdp

	       traceAll << this->GetMaxSize().GetValue() << "\n";	//estimated rtt
	   }
	   else
	   {
